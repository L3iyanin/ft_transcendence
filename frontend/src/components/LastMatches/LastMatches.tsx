...
const LastMatches : React.FC < {matches : IMatch[], isInProfile? : boolean, userId?: number, isForAllUsers?: boolean} > = ({ isInProfile, userId, isForAllUsers }) => {
	...
	useEffect(() => {
		if (userData && userData.user) {
			...
			getLastMatches(fetchedUserId, isForAllUsers)
				.then(res => {
					setLastMatches(res);
				})
				.catch(err => {
	...
	return (
		<section className={`${isInProfile ? "h-[368px]" : "h-[481px]"} xl:basis-6/12 basis-full  pl-8 pr-1 pt-6 rounded-2xl flex flex-col items-start gap-8 bg-dark-60 text-white`}>
			<h2 className="text-3xl font-bold">{t("homePage.LastMatches")}</h2>
			<section className={`${isInProfile ? "max-h-[320px]" : "max-h-[410px]"} w-full pr-8 overflow-y-auto`}>
