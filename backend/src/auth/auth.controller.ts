...
export class AuthController {
    ...
    async Auth42(@Req() req:Request, @Res() res: Response){
        ...
        const user = await this.authService.saveUserInDatabase(userData.userName, userData.fullName, avatarImage)
        const token = await this.authService.crietJwtToken(user.username, user.fullName)
